...
public class HBaseDaoUtil {
    ...
    // 关闭连接
    public  void close() {
        if (hconnectionFactory.getConnection() != null) {
            try {
                hconnectionFactory.getConnection().close();
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
    ...
    public void createTable(String tableName, Set<String> familyColumn) {
        ...
        try (Admin admin = hconnectionFactory.getConnection().getAdmin();) {
    ...
    public <T> List<T> queryScan(T obj, Map<String, String> param)throws Exception{
        ...
        try (Table table = hconnectionFactory.getConnection().getTable(TableName.valueOf(tableName)); Admin admin = hconnectionFactory.getConnection().getAdmin();){
    ...
    private void delete(List<Delete> deletes, String tableName) {
        try (Table table = hconnectionFactory.getConnection().getTable(TableName.valueOf(tableName));) {
            if (StringUtils.isBlank(tableName)) {
